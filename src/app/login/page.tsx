    ...
    if (validateForm()) {
      ...
      try {
        ...
        const { data, error } = await supabase
          ...
          .select("userid, email, passwordhash, salt, phone")
        ...
        const { userid, email, passwordhash, salt, phone } = data
        ...
        await supabase.from("User").update({ lastlogin: new Date().toISOString() }).eq("email", formData.email)
  ...
  return (
    <Layout>
      <div className="mx-auto mt-8 max-w-md rounded-lg bg-[#1a2b4b] p-6 shadow-lg">
        ...
        <div className="mt-4 text-sm text-center text-blue-300 hover:text-blue-200">Forgot password?</div>
